...
int load_game(Player* player, int* completed_traps) {// oyunu burda yuklerız
    ...
    if (fgets(line, sizeof(line), file) == NULL || sscanf(line, "%d", &player->health) != 1) {
        printf("Error reading player health.\n");
        fclose(file);
        return 0;
    }

    if (fgets(line, sizeof(line), file) == NULL || sscanf(line, "%d", &player->attack_power) != 1) {
        printf("Error reading attack power.\n");
        fclose(file);
        return 0;
    }

    if (fgets(line, sizeof(line), file) == NULL || sscanf(line, "%d", &player->inventory_count) != 1) {
        printf("Error reading inventory count.\n");
        fclose(file);
        return 0;
    }

    // Envanter elemanlarını oku
    for (int i = 0; i < player->inventory_count; i++) {
        if (fgets(line, sizeof(line), file) == NULL) {
            printf("Error reading inventory item %d.\n", i);
            fclose(file);
            return 0;
        }
        line[strcspn(line, "\n")] = '\0'; 
        strcpy(player->inventory[i], line);
    }

    // Tuzakları (completed_traps) oku
    for (int i = 0; i < MAX_TRAPS; i++) {
        if (fgets(line, sizeof(line), file) == NULL || sscanf(line, "%d", &completed_traps[i]) != 1) {
            printf("Error reading completed_traps[%d].\n", i);
...
int room3_chest(Player* player, Room* room, Item room3_items[]) { // istediği itemleri alır veya bırakır
    ...
    if (strcmp(look_choice, "look") == 0) {
        ...
        while (player->inventory_count < 7 && player->health > 0) {
            int item_choice;
